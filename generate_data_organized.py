...
def get_csbm(num_nodes, num_features, average_degree, mu, lamb, degree_corrected, num_classes):
    ...
    # print(p_matrix)
    # Generate SBM graph
    sbm_graph = nx.stochastic_block_model(block_sizes, p_matrix)
...
# lambda_range = torch.linspace(lamb_range[0],lamb_range[1],121)
...
for model_type in models:
    if model_type is  GraphTransformer:
        for mu in mu_range:
            for lamb in lambda_range:
                ...
                for run in range(args.runs):
                    ...
                    for batch in test_loader:
                        batch = batch.cuda()
                        out = model(batch)
                        accs.append(accuracy_1(torch.argmax(out,dim=-1),test_dataset[2]))
        ...
        for mu in mu_range:
            for lamb in lambda_range:
                ...
                for run in range(args.runs):
                    ...
                    accs.append(accuracy_1(torch.argmax(model(test_dataset[1],test_dataset[0]),dim=-1),test_dataset[2]))
                print(max(accs),lamb,mu)
                print(accs)
                store_acc(max(accs),lamb,mu,model_type,args.num_classes)
