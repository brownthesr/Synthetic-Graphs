...
def get_model_and_optimizer(model_type, num_features, hidden_layers, out_features):
...
for model_type in models:
    # Define a list for storing the accuracy
    # Note we are storing this in long form data to ensure compatability with pandas df 
    # eg: [[acc_1, lamb_1, mu_1],
    #      [acc_2, lambd_2, mu_2],...]
    all_accs = []   
    ...
    if model_type in [GCN, SAGE, GAT]:
        ...
        for mu in mu_range:
            for lamb in lambda_range:
                # Define accuracy list to max over
                sample_accs = []
                for run in range(args.runs):
                    # Get datasets
