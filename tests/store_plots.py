...
models = ["GCN","SAGE","GAT","transformer"]
num_class = [2,3,5,7]
dc_s = [True,False]

for model in models:
    for num_classes in num_class:
        for DC in dc_s:
            ...
            # ax_main.grid(False)
            if(DC):
                plt.savefig(f'results/{num_classes}_DC_{model}.png')
            else:
                plt.savefig(f'results/{num_classes}_{model}.png')
            plt.clf()
